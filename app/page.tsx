      ...
      <section className="relative w-full h-[92vh] min-h-[600px] overflow-hidden">
        ...
        <div className="absolute inset-0 z-10 flex flex-col justify-center items-center">
          <div className="text-center">
            ...
            <h1 className="text-[56px] font-extralight text-white tracking-[0.18em] leading-tight">FOCUS STATION</h1>
            <p className="text-[15px] font-light text-white/50 mt-4 tracking-[0.1em]">당신의 집중을 위한 공간</p>
            <div className="flex gap-4 justify-center mt-11">
              <Link href="/seats" className="px-10 py-3.5 border border-white/80 text-white text-[13px] font-medium tracking-[0.08em] no-underline hover:bg-white hover:text-[var(--text)] transition-all">좌석 예약</Link>
              <Link href="/timer" className="px-10 py-3.5 border border-white/30 text-white/60 text-[13px] font-normal tracking-[0.08em] no-underline hover:border-white/70 hover:text-white transition-all">공부 타이머</Link>
            </div>
          </div>
        </div>
      </section>

      <section className="max-w-[1000px] mx-auto px-8 py-24 text-center">
        <div className="text-[11px] font-medium text-[var(--text-tertiary)] tracking-[0.2em] mb-4">ABOUT THE SPACE</div>
        <h2 className="text-[28px] font-light text-[var(--text)] mb-5 leading-relaxed">조용하고 쾌적한 환경에서<br />온전히 나에게 집중하는 시간</h2>
        ...
        <div className="grid grid-cols-2 gap-0.5 mt-16 mb-0.5">
          {[
        ...
        <div className="grid grid-cols-4 gap-px mt-16 bg-[var(--border)] border border-[var(--border)]">
          {[
            ...
            <div key={i} className="bg-white p-8 text-left">
        ...
        <div className="grid grid-cols-4 gap-px bg-[var(--border)] border border-[var(--border)]">
          {ZONES.map((z) => (
            <div key={z.id} className="bg-white p-9 flex flex-col">
              ...
              <div>
                <span className="font-mono text-xl font-bold text-[var(--text)]">₩{z.price.toLocaleString()}</span>
